...
//
// `__APP_DEV__` is how src/lib/store.ts tells local dev (writes go to
// ./devfs-playground) from a real host. It is a `define`d global because
// `import.meta` is a parse error in files the immediately.run sandbox transpiles.
